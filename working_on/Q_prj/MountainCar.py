...
class NNetwork_Obj:
    ...
    def Net_graph(self,graph,net_input,targetOutput):
        ...
        W = tf.Variable(tf.random_uniform([input_dim,150],-0.001,0.001))
        B = tf.Variable(tf.random_uniform([150],-0.001,0.001))


        Wo = tf.Variable(tf.random_uniform([150,output_dim],-0.001,0.001))
...
class QLearning_OBJ:
    ...
    def training_exp_replay(self,cs_arr,a_arr,nr_arr,ns_arr,isEnd_arr=[False]):
    ...
    def experience_append(self,cs,a,nr,ns,isEnd=False,experience_limit=1500):
    ...
    rewardHist_offset =1
    rewardHist=[0,0,0]
    def experience_append_b(self,cs,a,nr,ns,isEnd=False,experience_limit=700):
        ...
        while True:
            ...
            if(stop_training == False):
                if end == False:#reduce reward value to prevent
                    qobj.experience_append(cs_vec,a,nr*0.05,ns_vec)
                    #dqobj.training_exp_replay([cs_vec],[a],[nr*0.05],[ns_vec])
                else:
                    turn_lifeTime=j
                    ccc=0
                    margin = 1
                    ...
                    qobj.experience_append(cs_vec,a,nr,ns_vec,True)
                    if(nr>0):
                        qobj.experience_append(cs_vec,a,nr,ns_vec,True)
                        qobj.experience_append(cs_vec,a,nr,ns_vec,True)
                        qobj.experience_append(cs_vec,a,nr,ns_vec,True)
